...
p7_DomainDecoding_Frameshift(const P7_PROFILE *gm, const P7_GMX *fwd, const P7_GMX *bck, P7_DOMAINDEF *ddef)
{
  ...
  for (i = 1; i <= L; i++) 
    { 
      ...
      if(i <= L-2) {
         njcp += n[i+2] + j[i+2] + c[i+2];
         njcp += n[i+1] + j[i+1] + c[i+1];
      } else if(i <= L-1) {
         njcp += n[i+1] + j[i+1] + c[i+1];
         njcp += n[i+1] + j[i+1] + c[i+1];
      } else { 

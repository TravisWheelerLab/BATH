...
*/
  ...
  if (cfg->qfmt != eslSQFILE_FASTA && cfg->qfmt != NHMMER_QFORMAT_HMM && hfp == NULL  ) { /*i.e. some MSA type; or unspecified, and HMM didn't work*/
    ...
    if (abc->type != eslAMINO) p7_Fail("fsphmmert only supports amino acid queryfiles; %s uses a different alphabet", cfg->queryfile);
  ...
  if (cfg->qfmt == eslSQFILE_FASTA || (hfp == NULL && qfp_msa == NULL)) {
    ...
    if (abc == NULL) {
      ...
      else if (q_type != eslAMINO) p7_Fail("fsphmmert only supports amino acid queryfiles; %s uses a different alphabet", cfg->queryfile);

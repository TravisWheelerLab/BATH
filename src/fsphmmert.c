...
#endif /*HMMER_THREADS*/
//TODO: edit command line processing
static int
...
//  int               force_single = ( esl_opt_IsOn(go, "--singlemx") ? TRUE : FALSE );
  ...
  if ( cfg->qfmt == NHMMER_QFORMAT_HMM || cfg->qfmt == NHMMER_QFORMAT_UNKNOWN ) {
      ...
      qhstatus = p7_hmmfile_Read(hfp, &abcAMINO, &hmm);
      if (abcAMINO->type != eslAMINO) p7_Fail("fsphmmert only supports amino acid HMMs; %s uses a different alphabet", cfg->queryfile);
  ...
  if (cfg->qfmt != eslSQFILE_FASTA && cfg->qfmt != NHMMER_QFORMAT_HMM && hfp == NULL  ) { /*i.e. some MSA type; or unspecified, and HMM didn't work*/
    ...
    status = esl_msafile_Open(&abcAMINO, cfg->queryfile, NULL, qfmt_arg, NULL, &qfp_msa);

    if (abcAMINO == NULL) p7_Fail ("Unable to guess alphabet for query file %s\n", cfg->queryfile);
...
/* (3)
  ...
  if (cfg->qfmt == eslSQFILE_FASTA || (hfp == NULL && qfp_msa == NULL)) {
    ...
    if (abcAMINO == NULL) {
      ...
      abcAMINO     = esl_alphabet_Create(q_type);
    }
    esl_sqfile_SetDigital(qfp_sq, abcAMINO);

    // read first sequence
    qsq  = esl_sq_CreateDigital(abcAMINO);
    qhstatus = esl_sqio_Read(qfp_sq, qsq);
    if (qhstatus != eslOK) p7_Fail("reading sequence from file %s (%d)\n", cfg->queryfile, qhstatus);
  }

  if (! (abcAMINO->type == eslAMINO) )
    p7_Fail("Invalid alphabet type in query for fsphmmert. Expect AMINO ACIDS\n");
  ...
  */
  ...
  if (esl_opt_IsOn(go, "--bgfile")) {
    bg_manual = p7_bg_Create(abcAMINO);
...
for (i = 0; i < info->th->N; i++)
	...
	if (tblfp)     p7_tophits_TabularTargets(tblfp,    hmm->name, hmm->acc, info->th, info->pli, (nquery == 1));
      ...
      if (afp) {
          ...
          if (p7_tophits_Alignment(info->th, abcAMINO, NULL, NULL, 0, p7_DEFAULT, &msa) == eslOK) 
	    {
...
serial_loop(WORKER_INFO *info, ID_LENGTH_LIST *id_length_list, ESL_SQFILE *dbfp, char *firstseq_key, int n_targetseqs)
{
 ...
 /* Set up the genetic code. Default = NCBI 1, the standard code; allow ORFs to start at any aa   */
  ...
  esl_gencode_Set(gcode, esl_opt_GetInteger(info->go, "-c"));  // default = 1, the standard genetic code

  if      (esl_opt_GetBoolean(info->go, "-m"))   esl_gencode_SetInitiatorOnlyAUG(gcode);
  else if (! esl_opt_GetBoolean(info->go, "-M")) esl_gencode_SetInitiatorAny(gcode);      // note this is the default, if neither -m or -M are set
   ...
   */
  wrk = esl_gencode_WorkstateCreate(info->go, gcode);
...
thread_loop(WORKER_INFO *info, ID_LENGTH_LIST *id_length_list, ESL_THREADS *obj, ESL_WORK_QUEUE *queue, ESL_SQFILE *dbfp, char *firstseq_key, int n_targetseqs)
{
  ...
  void         *newBlock;
  ESL_ALPHABET    *abcDNA = esl_alphabet_Create(eslDNA); 
  ESL_SQ      *tmpsq = esl_sq_CreateDigital(abcDNA);
...
pipeline_thread(void *arg)
{
 ...
 if (status != eslOK) esl_fatal("Work queue worker failed");
  ...
  while (block->count > 0)
  {
	...
	p7_Pipeline_Frameshift(info->pli, info->om, info->gm, info->scoredata, info->bg, info->th, block->first_seqidx + i, dbsq, p7_NOCOMPLEMENT, gcode, wrk);
        p7_pipeline_fs_Reuse(info->pli); // prepare for next search
	...
	if(wrk->orf_block != NULL) {
          ...
          p7_Pipeline_Frameshift(info->pli, info->om, info->gm, info->scoredata, info->bg, info->th, block->first_seqidx + i, dbsq, p7_COMPLEMENT,gcode,wrk);
          p7_pipeline_fs_Reuse(info->pli); // prepare for next search

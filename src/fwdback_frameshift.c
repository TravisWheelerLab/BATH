...
/* Function:  p7_Forward_Frameshift_Spliced()
 * Synopsis:  The Frameshift-Aware and Spliced Forward algorithm.
 *
 * Purpose:   The Forward dynamic programming algorithm for frameshift
 *            aware, spliced, and  translated comparison between a dna 
 *            sequence and an amino acid HMM. 
 *
 *            Given a digital sequence <dsq> of length <L>, a profile
 *            <gm>, and DP matrix <gx> allocated for at least <gm->M>
 *            by <L> cells; calculate the probability of the sequence
 ...
 */
int
p7_Forward_Frameshift_Spliced(const ESL_DSQ *dsq, const ESL_GENCODE *gcode, int L, const P7_FS_PROFILE *gm_fs, P7_GMX *gx, float *opt_sc)
{ 
  ...
  XMX_FS(0,p7G_E) = XMX_FS(0,p7G_J) = XMX_FS(0,p7G_C) = -eslINFINITY;
  for (k = 0; k <= M; k++) {
    MMX_SP_FS(0,k,p7G_C0) = MMX_SP_FS(0,k,p7G_C1) = MMX_SP_FS(0,k,p7G_C2) = MMX_SP_FS(0,k,p7G_C3) =
    MMX_SP_FS(0,k,p7G_C4) = MMX_SP_FS(0,k,p7G_C5) = IMX_SP_FS(0,k)        = DMX_SP_FS(0,k)        = -eslINFINITY;
    RMX_SP_FS(0,k) = PMX_SP_FS(0,k) = -eslINFINITY;    
  }
  ...
  for(i = 1; i < 5; i++)
  {
    ...
    MMX_SP_FS(i,0,p7G_C0) = MMX_SP_FS(i,0,p7G_C1) = MMX_SP_FS(i,0,p7G_C2) = MMX_SP_FS(i,0,p7G_C3) =
    MMX_SP_FS(i,0,p7G_C4) = MMX_SP_FS(i,0,p7G_C5) = IMX_SP_FS(i,0)        = DMX_SP_FS(i,0)        = -eslINFINITY;
    RMX_SP_FS(i,0) = PMX_SP_FS(i,0) = -eslINFINITY;
    XMX_FS(i,p7G_E) = -eslINFINITY;

    /* Initialization of the states reacheable at row i */
    for (k = 1; k < M; k++)
    {
      IVX(i,k,p7P_C1) = p7_FLogsum(MMX_SP_FS(i-1,k-1,p7G_C0) + TSC(p7P_MM,k-1),
                        p7_FLogsum(IMX_SP_FS(i-1,k-1)        + TSC(p7P_IM,k-1),
                        p7_FLogsum(DMX_SP_FS(i-1,k-1)        + TSC(p7P_DM,k-1),
                                   XMX_FS(i-1,p7G_B)         + TSC(p7P_BM,k-1))));
	
      MMX_SP_FS(i,k,p7G_C1) = IVX(i,k,p7P_C1) + p7P_MSC_CODON(gm_fs, k, c1);

      if( i > 1 ) MMX_SP_FS(i,k,p7G_C2) = IVX(i,k,p7P_C2) + p7P_MSC_CODON(gm_fs, k, c2);
      else        MMX_SP_FS(i,k,p7G_C2) = -eslINFINITY;

      if( i > 2 ) MMX_SP_FS(i,k,p7G_C3) = IVX(i,k,p7P_C3) + p7P_MSC_CODON(gm_fs, k, c3);
      else        MMX_SP_FS(i,k,p7G_C3) = -eslINFINITY;

      if( i > 3 ) MMX_SP_FS(i,k,p7G_C4) = IVX(i,k,p7P_C4) + p7P_MSC_CODON(gm_fs, k, c4);
      else        MMX_SP_FS(i,k,p7G_C4) = -eslINFINITY;

      MMX_SP_FS(i,k,p7G_C5) = -eslINFINITY; 

      MMX_SP_FS(i,k,p7G_C0) =  p7_FLogsum(MMX_SP_FS(i,k,p7G_C1),
                               p7_FLogsum(MMX_SP_FS(i,k,p7G_C2),
                               p7_FLogsum(MMX_SP_FS(i,k,p7G_C3),
                                          MMX_SP_FS(i,k,p7G_C4))));

      /* insert state */
      if ( i > 2)
        IMX_SP_FS(i,k) = p7_FLogsum(MMX_SP_FS(i-3,k,p7G_C0) + TSC(p7P_MI,k),
                                    IMX_SP_FS(i-3,k)        + TSC(p7P_II,k));
      else
        IMX_SP_FS(i,k) = -eslINFINITY;

      /* delete state */
      DMX_SP_FS(i,k) = p7_FLogsum(MMX_SP_FS(i,k-1,p7G_C0) + TSC(p7P_MD,k-1),
                                  DMX_SP_FS(i,k-1)        + TSC(p7P_DD,k-1));

      RMX_SP_FS(i,k) = PMX_SP_FS(i,k) = -eslINFINITY;

      /* E state update */
      XMX_FS(i,p7G_E) = p7_FLogsum(MMX_SP_FS(i,k,p7G_C0) + esc,
                        p7_FLogsum(DMX_SP_FS(i,k)        + esc, 
                                   XMX_FS(i,p7G_E)));
    }

    IVX(i,M,p7P_C1) = p7_FLogsum(MMX_SP_FS(i-1,M-1,p7G_C0)   + TSC(p7P_MM,M-1),
                      p7_FLogsum(IMX_SP_FS(i-1,M-1)          + TSC(p7P_IM,M-1),
                      p7_FLogsum(DMX_SP_FS(i-1,M-1)          + TSC(p7P_DM,M-1),
                                 XMX_FS(i-1,p7G_B)        + TSC(p7P_BM,M-1))));

    MMX_SP_FS(i,M,p7G_C1) = IVX(i,M,p7P_C1) + p7P_MSC_CODON(gm_fs, M, c1);

    if( i > 1 )
      MMX_SP_FS(i,M,p7G_C2) = IVX(i,M,p7P_C2) + p7P_MSC_CODON(gm_fs, M, c2);
    else
      MMX_SP_FS(i,M,p7G_C2) = -eslINFINITY;
    if( i > 2 )
      MMX_SP_FS(i,M,p7G_C3) = IVX(i,M,p7P_C3) + p7P_MSC_CODON(gm_fs, M, c3);
    else
      MMX_SP_FS(i,M,p7G_C3) = -eslINFINITY;
    if( i > 3 )
      MMX_SP_FS(i,M,p7G_C4) = IVX(i,M,p7P_C4) + p7P_MSC_CODON(gm_fs, M, c4);
    else
      MMX_SP_FS(i,M,p7G_C4) = -eslINFINITY;
    MMX_SP_FS(i,M,p7G_C5)   = -eslINFINITY;

    MMX_SP_FS(i,M,p7G_C0) =  p7_FLogsum(p7_FLogsum(MMX_SP_FS(i,M,p7G_C1), MMX_SP_FS(i,M,p7G_C2)),
                                     p7_FLogsum(MMX_SP_FS(i,M,p7G_C3), MMX_SP_FS(i,M,p7G_C4)));

    /* insert state */
    IMX_SP_FS(i,M) = -eslINFINITY;
      
    /* delete state */
    DMX_SP_FS(i,M) = p7_FLogsum(MMX_SP_FS(i,M-1,p7G_C0) + TSC(p7P_MD,M-1),
                                DMX_SP_FS(i,M-1)        + TSC(p7P_DD,M-1));

    RMX_SP_FS(i,M) = PMX_SP_FS(i,M) = -eslINFINITY;

    /* E state update */
    XMX_FS(i,p7G_E) = p7_FLogsum(MMX_SP_FS(i,M,p7G_C0),
                      p7_FLogsum(DMX_SP_FS(i,M), 
                                 XMX_FS(i,p7G_E)));
  ...
  for (i = 5; i <= L; i++) 
  {
    ...
    MMX_SP_FS(i,0,p7G_C0) = -eslINFINITY;
    MMX_SP_FS(i,0,p7G_C1) = -eslINFINITY;
    MMX_SP_FS(i,0,p7G_C2) = -eslINFINITY;
    MMX_SP_FS(i,0,p7G_C3) = -eslINFINITY;
    MMX_SP_FS(i,0,p7G_C4) = -eslINFINITY;
    MMX_SP_FS(i,0,p7G_C5) = -eslINFINITY;
    IMX_SP_FS(i,0) = -eslINFINITY;
    DMX_SP_FS(i,0) = -eslINFINITY;
    RMX_SP_FS(i,0) = PMX_SP_FS(i,0) = -eslINFINITY; 
    XMX_FS(i, p7G_E) = -eslINFINITY;
    ...
    {  
      ...
      MMX_SP_FS(i,k,p7G_C2) = IVX(i,k,p7P_C2) + p7P_MSC_CODON(gm_fs, k, c2);
    
      MMX_SP_FS(i,k,p7G_C3) = IVX(i,k,p7P_C3) + p7P_MSC_CODON(gm_fs, k, c3);
      
      MMX_SP_FS(i,k,p7G_C4) = IVX(i,k,p7P_C4) + p7P_MSC_CODON(gm_fs, k, c4);

      MMX_SP_FS(i,k,p7G_C5) = IVX(i,k,p7P_C5) + p7P_MSC_CODON(gm_fs, k, c5); 

      MMX_SP_FS(i,k,p7G_C0) =  p7_FLogsum(MMX_SP_FS(i,k,p7G_C1),
                               p7_FLogsum(MMX_SP_FS(i,k,p7G_C2),
                               p7_FLogsum(MMX_SP_FS(i,k,p7G_C3),
                               p7_FLogsum(MMX_SP_FS(i,k,p7G_C4),
                                          MMX_SP_FS(i,k,p7G_C5)))));
    ...
    if(i > 11) IVX(i,M,p7P_C1) = p7_FLogsum(IVX(i,M,p7P_C1), PMX_SP_FS(i-1,M-1));
    ...
    MMX_SP_FS(i,M,p7G_C5) = IVX(i,M,p7P_C5) + p7P_MSC_CODON(gm_fs, M, c5);

    MMX_SP_FS(i,M,p7G_C0) =  p7_FLogsum(MMX_SP_FS(i,M,p7G_C1),
                             p7_FLogsum(MMX_SP_FS(i,M,p7G_C2),
                             p7_FLogsum(MMX_SP_FS(i,M,p7G_C3),
                             p7_FLogsum(MMX_SP_FS(i,M,p7G_C4),
                                        MMX_SP_FS(i,M,p7G_C5)))));
    ...
    /* unrolled E state update */
    XMX_FS(i,p7G_E) = p7_FLogsum(p7_FLogsum(MMX_SP_FS(i,M,p7G_C0),
                                            DMX_SP_FS(i,M)),
                                            XMX_FS(i,p7G_E));
...
/* Function:  p7_Forward_Parser_Frameshift()
...
{ 
  ...
  for(i = 1; i < 5; i++)
  {
	...
	/* For every row the 1 nuc codon translations must be reclacuated (i-1).
 	 ...
 	 * i-2 becoming i-3 and so on */	
      IVX(i,k,p7P_C1) = p7_FLogsum(MMX(prev1,k-1)   + TSC(p7P_MM,k-1),
                        p7_FLogsum(IMX(prev1,k-1)   + TSC(p7P_IM,k-1),
                        p7_FLogsum(DMX(prev1,k-1)   + TSC(p7P_DM,k-1),
                                   XMX(i-1,p7G_B)   + TSC(p7P_BM,k-1))));
	
      MMX(curr,k) =                           IVX(i,k,p7P_C1) + p7P_MSC_CODON(gm_fs, k, c1);
      if( i > 1 )
        MMX(curr,k) = p7_FLogsum(MMX(curr,k), IVX(i,k,p7P_C2) + p7P_MSC_CODON(gm_fs, k, c2));
      if( i > 2 )
        MMX(curr,k) = p7_FLogsum(MMX(curr,k), IVX(i,k,p7P_C3) + p7P_MSC_CODON(gm_fs, k, c3));
      if( i > 3 )
        MMX(curr,k) = p7_FLogsum(MMX(curr,k), IVX(i,k,p7P_C4) + p7P_MSC_CODON(gm_fs, k, c4));
   ...
   /* insert state */
    ...
    MMX(curr,M) =                           IVX(i,M,p7P_C1) + p7P_MSC_CODON(gm_fs, M, c1);

    if( i > 1 )
      MMX(curr,M) = p7_FLogsum(MMX(curr,M), IVX(i,M,p7P_C2) + p7P_MSC_CODON(gm_fs, M, c2));
    if( i > 2 )
      MMX(curr,M) = p7_FLogsum(MMX(curr,M), IVX(i,M,p7P_C3) + p7P_MSC_CODON(gm_fs, M, c3));
    if( i > 3 )
      MMX(curr,M) = p7_FLogsum(MMX(curr,M), IVX(i,M,p7P_C4) + p7P_MSC_CODON(gm_fs, M, c4));
  ...
  for (i = 5; i <= L; i++) 
  {
    ...
    {  
      ...
      MMX(curr,k) =  p7_FLogsum(MMX(curr,k), IVX(i,k,p7P_C5) + p7P_MSC_CODON(gm_fs, k, c5)); 
    ...
    MMX(curr,M) = p7_FLogsum(MMX(curr,M), IVX(i,M,p7P_C5) + p7P_MSC_CODON(gm_fs, M, c5));
...
p7_Backward_Frameshift(const ESL_DSQ *dsq, const ESL_GENCODE *gcode, int L, const P7_FS_PROFILE *gm_fs, P7_GMX *gx, float *opt_sc)
{
  ...
  for (i = L-1; i > L-5; i--)
  {
    ...
    iv[1] =                     MMX(i+1,1) + p7P_MSC_CODON(gm_fs, 1, c1);

    if( i < L-1 )
      iv[1] = p7_FLogsum(iv[1], MMX(i+2,1) + p7P_MSC_CODON(gm_fs, 1, c2));
     
    if( i < L-2 )
      iv[1] = p7_FLogsum( iv[1], MMX(i+3,1) + p7P_MSC_CODON(gm_fs, 1, c3));

    if( i < L-3 )
      iv[1] = p7_FLogsum( iv[1], MMX(i+4,1) + p7P_MSC_CODON(gm_fs, 1, c4));
  
    XMX(i,p7G_B)   =  iv[1] + TSC(p7P_BM,0);
 
    for (k = 2; k <= M; k++) 
    {
      iv[k]  =                     MMX(i+1,k) + p7P_MSC_CODON(gm_fs, k, c1);
      
      if( i < L-1 )
        iv[k] = p7_FLogsum( iv[k], MMX(i+2,k) + p7P_MSC_CODON(gm_fs, k, c2));

      if( i < L-2 )
        iv[k] = p7_FLogsum( iv[k], MMX(i+3,k) + p7P_MSC_CODON(gm_fs, k, c3));

      if( i < L-3 )
        iv[k] = p7_FLogsum( iv[k], MMX(i+4,k) + p7P_MSC_CODON(gm_fs, k, c4));
  ...
  for (i = L-5; i > 0; i--)
  {
    ...
    for (k = 2; k <= M; k++) {
      iv[k] = p7_FLogsum( MMX(i+1,k) + p7P_MSC_CODON(gm_fs, k, c1),
  ...
  for (k = 2; k <= M; k++) 
  {
   iv[k] =  p7_FLogsum( MMX(1,k) + p7P_MSC_CODON(gm_fs, k, c1),
...
p7_BackwardParser_Frameshift(const ESL_DSQ *dsq, const ESL_GENCODE *gcode, int L, const P7_FS_PROFILE *gm_fs, P7_GMX *gx, float *opt_sc)
{
  ...
  for (i = L-1; i > L-5; i--)
  {
    ...
    iv[1] =                     MMX(prev1,1) + p7P_MSC_CODON(gm_fs, 1, c1);

    if( i < L-1 )
      iv[1] = p7_FLogsum(iv[1], MMX(prev2,1) + p7P_MSC_CODON(gm_fs, 1, c2));
 
    if( i < L-2 )
      iv[1] = p7_FLogsum( iv[1], MMX(prev3,1) + p7P_MSC_CODON(gm_fs, 1, c3));

    if( i < L-3 )
      iv[1] = p7_FLogsum( iv[1], MMX(prev4,1) + p7P_MSC_CODON(gm_fs, 1, c4));
  
    XMX(i,p7G_B)   =  iv[1] + TSC(p7P_BM,0);

     for (k = 2; k <= M; k++) 
    {
      iv[k]  = MMX(prev1,k) + p7P_MSC_CODON(gm_fs, k, c1);
      
      if( i < L-1 )
        iv[k] = p7_FLogsum( iv[k], MMX(prev2,k) + p7P_MSC_CODON(gm_fs, k, c2));

      if( i < L-2 )
        iv[k] = p7_FLogsum( iv[k], MMX(prev3,k) + p7P_MSC_CODON(gm_fs, k, c3));

      if( i < L-3 )
        iv[k]  = p7_FLogsum( iv[k], MMX(prev4,k) + p7P_MSC_CODON(gm_fs, k, c4));
    ...
    for (k = M-1; k >= 1; k--)
    {
      ...
      if (i < L-2 )
        ...
        IMX(curr,k) = iv[k+1]            + TSC(p7P_IM,k);
    }
  ...
  for (i = L-5; i > 0; i--)
  {
    ...
    iv[1] = p7_FLogsum( MMX(prev1,1) + p7P_MSC_CODON(gm_fs, 1, c1), 
    ...
    for (k = 2; k <= M; k++) {
      iv[k] = p7_FLogsum( MMX(prev1,k) + p7P_MSC_CODON(gm_fs, k, c1),

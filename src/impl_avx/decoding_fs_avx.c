/* Frameshift posterior decoding algorithms; AVX2 implementations.
 * Ported from impl_avx/decoding_fs_sse.c with _avx suffix for runtime dispatch.
 *
 * Contents:
 *   1. p7_Decoding_Frameshift_avx()
 *   2. p7_DomainDecoding_Frameshift_avx()
 */
#include "p7_config.h"
...
/* Function:  p7_Decoding_Frameshift_avx()
 *
 * Purpose:   AVX2 implementation of frameshift posterior decoding.
 ...
 */
int
p7_Decoding_Frameshift_avx(const P7_FS_OPROFILE *om_fs, P7_OMX *fwd, const P7_OMX *bck)
{
  int    L = fwd->L;
  int    M = om_fs->M;
  int    Q = p7O_NQF_AVX(M);
  ...
  for (i = 1; i <= L; i++)
    {
      ...
      for (q = 0; q < Q; q++)
        {
          ...
          dpc[q * p7X_NSCELLS_FS + p7X_FS_I] = _mm256_mul_ps(fI, bI);
          for (c = 0; c < 6; c++)
            dpc[q * p7X_NSCELLS_FS + p7X_FS_M + c] = _mm256_mul_ps(fC[c], bM);

          /* Only C0 and I contribute to the per-row normalization denominator */
          dv = _mm256_add_ps(dv, _mm256_add_ps(dpc[q * p7X_NSCELLS_FS + p7X_FS_M],
                                                dpc[q * p7X_NSCELLS_FS + p7X_FS_I]));
        }

      /* Horizontal sum of dv to get scalar raw MDI denom */
      raw_denom = 0.0f;
      esl_avx_hsum_ps(dv, &raw_denom);
      ...
      scv = _mm256_set1_ps(factor_mdi * inv_denom);
      for (q = 0; q < Q; q++)
        for (c = 0; c < p7X_NSCELLS_FS; c++)
          fwd->dpf_avx[i][q * p7X_NSCELLS_FS + c] = _mm256_mul_ps(fwd->dpf_avx[i][q * p7X_NSCELLS_FS + c], scv);
...
/* Function:  p7_DomainDecoding_Frameshift_avx()
 *
 * Purpose:   AVX2 implementation of frameshift-aware posterior decoding of
 ...
 */
int
p7_DomainDecoding_Frameshift_avx(const P7_FS_OPROFILE *om_fs, const P7_OMX *oxf, const P7_OMX *oxb,
                                   P7_DOMAINDEF *ddef)
{
  int    L   = oxf->L;
  float *log_sfwd = NULL;

/* Frameshift null2 model; AVX2 implementation.
 * Ported from null2_fs_sse.c with _avx suffix for runtime dispatch.
 *
 * Contents:
 *   1. p7_Null2_fs_ByExpectation_avx()
 */
#include "p7_config.h"
...
/* Function:  p7_Null2_fs_ByExpectation_avx()
 *
 * Purpose:   AVX2 implementation of null2 estimation by expectation for
 *            the frameshift model. Uses the 8-cell FS layout posterior
 *            matrix <pp> (from p7_Decoding_Frameshift_avx()) and the
 ...
 */
int
p7_Null2_fs_ByExpectation_avx(const P7_FS_OPROFILE *om_fs, P7_OMX *pp, float *null2)
{
  int      M    = om_fs->M;
  int      Ld   = pp->L;
  int      Q    = p7O_NQF_AVX(M);
  ...
  /* Determine the amino acid emission row offset in rfv_avx[] */
   ...
   */
  memcpy(pp->dpf_avx[0], pp->dpf_avx[1], sizeof(__m256) * p7X_NSCELLS_FS * Q);
  ...
  for (i = 2; i <= Ld; i++)
    {
      for (q = 0; q < Q; q++)
        {
          MMO_FS(pp->dpf_avx[0], q, p7X_FS_C0) = _mm256_add_ps(MMO_FS(pp->dpf_avx[i], q, p7X_FS_C0),
                                                                  MMO_FS(pp->dpf_avx[0], q, p7X_FS_C0));
          IMO_FS(pp->dpf_avx[0], q)             = _mm256_add_ps(IMO_FS(pp->dpf_avx[i], q),
                                                                  IMO_FS(pp->dpf_avx[0], q));
        }
  ...
  for (q = 0; q < Q; q++)
    {
      MMO_FS(pp->dpf_avx[0], q, p7X_FS_C0) = _mm256_mul_ps(MMO_FS(pp->dpf_avx[0], q, p7X_FS_C0), sv);
      IMO_FS(pp->dpf_avx[0], q)             = _mm256_mul_ps(IMO_FS(pp->dpf_avx[0], q),             sv);
    }
  XMXo(0, p7X_N) *= norm;
  XMXo(0, p7X_C) *= norm;
  XMXo(0, p7X_J) *= norm;
   ...
   * Amino acid emission odds are in rfv_avx[amino_offset + x].
  ...
  for (x = 0; x < om_fs->abc->K; x++)
    {
      ...
      rp = om_fs->rfv_avx[amino_offset + x];
      for (q = 0; q < Q; q++)
        {
          sv = _mm256_add_ps(sv, _mm256_mul_ps(MMO_FS(pp->dpf_avx[0], q, p7X_FS_C0), *rp)); rp++;
          sv = _mm256_add_ps(sv, IMO_FS(pp->dpf_avx[0], q));  /* insert odds implicitly 1.0 */
        }
      null2[x] = 0.0f;

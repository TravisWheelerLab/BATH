/* p7_fs_oprofile_sse.c — SSE (128-bit) implementations of P7_FS_OPROFILE management.
 *
 ...
 *
 * Ported from impl_sse/p7_fs_oprofile.c with:
 ...
 *   - Benchmark/unit-test/example drivers omitted
 */
#include "p7_config.h"
...
/* Function:  p7_fs_oprofile_Create_sse()
 * Synopsis:  Allocate an optimized frameshift profile structure (SSE).
 */
P7_FS_OPROFILE *
p7_fs_oprofile_Create_sse(int allocM, const ESL_ALPHABET *abc, int codon_lengths)
{
  int             status;
  ...
  /* AVX/AVX-512 fields — NULL/0 since this is the SSE implementation */
#ifdef eslENABLE_AVX
  ...
  /* Raw memory (+15 bytes for manual 16-byte alignment) */
...
/* Function:  p7_fs_oprofile_Destroy_sse()
 * Synopsis:  Free an optimized frameshift profile (SSE fields only).
 */
void
p7_fs_oprofile_Destroy_sse(P7_FS_OPROFILE *om_fs)
{
...
/* Function:  p7_fs_oprofile_Clone_sse()
 * Synopsis:  Shallow clone for multithreading (SSE).
 */
P7_FS_OPROFILE *
p7_fs_oprofile_Clone_sse(const P7_FS_OPROFILE *om_fs)
{
  int             status;
 ...
 ERROR:
  p7_fs_oprofile_Destroy_sse(om2);
  return NULL;
}


/*****************************************************************
 * 2. Conversion from P7_FS_PROFILE to P7_FS_OPROFILE (SSE).
 *****************************************************************/
...
/* Function:  p7_fs_oprofile_Convert_sse()
 */
int
p7_fs_oprofile_Convert_sse(const P7_FS_PROFILE *gm_fs, P7_FS_OPROFILE *om_fs)
{
  int status, z;
...
/* Function:  p7_fs_oprofile_Convert_Log_sse()
 */
int
p7_fs_oprofile_Convert_Log_sse(const P7_FS_PROFILE *gm_fs, P7_FS_OPROFILE *om_fs)
{
  int status, z;
...
/* Function:  p7_fs_oprofile_SubConvert_Log_sse()
 */
int
p7_fs_oprofile_SubConvert_Log_sse(const P7_FS_PROFILE *gm_fs, P7_FS_OPROFILE *om_fs, int k_start, int k_end)
{
  ...
  if (nq > om_fs->allocQ4) ESL_EXCEPTION(eslEINVAL, "optimized profile is too small to hold sub-region");
...
/*****************************************************************
 * 3. Reconfig functions (SSE).
 *****************************************************************/

int
p7_fs_oprofile_ReconfigLength_sse(P7_FS_OPROFILE *om_fs, int L)
{
...
int
p7_fs_oprofile_ReconfigLength_Log_sse(P7_FS_OPROFILE *om_fs, int L)
{
  float pmove = (2.0f + om_fs->nj) / ((float)L + 2.0f + om_fs->nj);
  float ploop = 1.0f - pmove;
...
int
p7_fs_oprofile_ReconfigMultihit_sse(P7_FS_OPROFILE *om_fs, int L)
{
...
int
p7_fs_oprofile_ReconfigUnihit_sse(P7_FS_OPROFILE *om_fs, int L)
{
...
int
p7_fs_oprofile_Logify_sse(P7_FS_OPROFILE *om_fs)
{

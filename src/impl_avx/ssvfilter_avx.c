/* SSV filter AVX2 implementation.
 * Ported from ssvfilter_sse.c using 256-bit vectors.
 * See ssvfilter_sse.c for a detailed explanation of the multi-band
 * macro scheme; this file uses the same approach with _mm256_ intrinsics.
 *
 * Contents:
...
/* Same register budget as SSE: 14 bands on x86-64 (16 YMM regs,
 * minus 2 for xEv and beginv). */
#ifdef __x86_64__
#define MAX_BANDS 14
#else
#define MAX_BANDS 6
...
get_xE_avx(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om)
{
  ...
  return esl_avx_hmax_epu8(xEv);
}


/* Function:  p7_SSVFilter_avx()
 * Synopsis:  SSV filter, AVX2 path.
 *

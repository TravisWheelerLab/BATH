/* Frameshift aware posterior decoding algorithms; NEON versions.
 *
 * Contents:
 ...
 * Synopsis:  NEON posterior decoding of residue assignments; frameshift full-matrix version.
 *
 * Purpose:   NEON equivalent of <p7_GDecoding_Frameshift()>.  Given filled forward
 ...
 *            the generic <p7_GDecoding_Frameshift()>.  Only M_C0 and I contribute
  ...
  for (i = 1; i <= L; i++)
    {
      ...
      for (q = 0; q < Q; q++)
        {
          ...
          dpc[q * p7X_NSCELLS_FS + p7X_FS_I] = vmulq_f32(fI, bI);
          for (c = 0; c < 6; c++)
            dpc[q * p7X_NSCELLS_FS + p7X_FS_M + c] = vmulq_f32(fC[c], bM);

          /* Only C0 and I contribute to the per-row normalization denominator */
          dv = vaddq_f32(dv, vaddq_f32(dpc[q * p7X_NSCELLS_FS + p7X_FS_M],
                                        dpc[q * p7X_NSCELLS_FS + p7X_FS_I]));
        }

      /* Horizontal sum of dv to get scalar raw MDI denom */
      esl_neon_hsum_float((esl_neon_128f_t){ .f32x4 = dv }, &raw_denom);
      ...
      scv = vmovq_n_f32(factor_mdi * inv_denom);
      for (q = 0; q < Q; q++)
        for (c = 0; c < p7X_NSCELLS_FS; c++)
          fwd->dpf[i][q * p7X_NSCELLS_FS + c] = vmulq_f32(fwd->dpf[i][q * p7X_NSCELLS_FS + c], scv);
 ...
 * Synopsis:  NEON posterior decoding of domain location for frameshift-aware alignments.
 *
 * Purpose:   Identical to <p7_GDomainDecoding_Frameshift()> except that <om_fs>,
 *            <oxf>, <oxb> are NEON optimized versions. See
 *            <p7_GDomainDecoding_Frameshift()> documentation for more info.
 *
...
/*
   gcc -g -O3 -march=armv8-a -std=gnu99 -o decoding_fs_benchmark -I.. -L.. -I../../easel -L../../easel -Dp7DECODING_FS_BENCHMARK decoding_fs.c -lhmmer -leasel -lm
...
static char banner[] = "benchmark driver for posterior residue decoding, NEON frameshift version";
 ...
 * forward/backward to produce a generic PP matrix (gx1), and the NEON full-matrix
 * forward/backward to produce a NEON PP matrix (fwd).  We compare the special-state
  ...
  while (N--)
    {
      ...
      /* Generic reference: p7_GDecoding_Frameshift overwrites gx1 with PP  */
      ...
      /* NEON under test: p7_Decoding_Frameshift overwrites fwd with PP  */
      ...
      /* --- Deconvert NEON PP to generic layout and compare --- */
...
#ifdef p7DECODING_FS_TESTDRIVE
/*
  gcc -g -Wall -march=armv8-a -std=gnu99 -o decoding_fs_utest -I.. -L.. -I../../easel -L../../easel -Dp7DECODING_FS_TESTDRIVE decoding_fs.c -lhmmer -leasel -lm
  ./decoding_fs_utest
*/
#include "p7_config.h"
...
static char banner[] = "test driver for NEON frameshift domain definition";

 ...
 *               and stripe q (0..allocQ4-1), each float32x4_t vector
 *               holds match emission scores for four model positions:
 *               k = q+1, q+1+Q4, q+1+2*Q4, q+1+3*Q4  (where Q4 = p7O_NQF(M))
 *
 *   For example, the emission layout for an M=14 model (Q4=4, NEON float):
 *
...
typedef struct p7_fs_oprofile_s {
  /* Forward/Backward: IEEE754 single-precision floats, 4x float32x4_t vectors  */
  float32x4_t **rfv;          /* match emission scores [c][q]                   */
                              /* c = 0..p7P_MAXCODONS#+Kp-1 (codon/aa index)    */
                              /* q = 0..allocQ4-1 (stripe index)                */
  float32x4_t  *tfv;          /* transition score blocks [p7O_NTRANS * allocQ4] */

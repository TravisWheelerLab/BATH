/* "null2" frameshift model, biased composition correction; NEON implementations.
 *
 * Contents:
 ...
 * Synopsis:  Calculate null2 model from posterior probabilities; frameshift NEON version.
 *
 * Purpose:   Identical to <p7_GNull2_fs_ByExpectation()> except that
 *            <om_fs>, <pp> are NEON optimized versions of the frameshift
...
p7_Null2_fs_ByExpectation(const P7_FS_OPROFILE *om_fs, P7_OMX *pp, float *null2)
{
   ...
   */
  memcpy(pp->dpf[0], pp->dpf[1], sizeof(float32x4_t) * p7X_NSCELLS_FS * Q);
  ...
  for (i = 2; i <= Ld; i++)
    {
      for (q = 0; q < Q; q++)
        {
          MMO_FS(pp->dpf[0], q, p7X_FS_C0) = vaddq_f32(MMO_FS(pp->dpf[i], q, p7X_FS_C0),
                                                         MMO_FS(pp->dpf[0], q, p7X_FS_C0));
          IMO_FS(pp->dpf[0], q)             = vaddq_f32(IMO_FS(pp->dpf[i], q),
                                                         IMO_FS(pp->dpf[0], q));
        }
  ...
  for (q = 0; q < Q; q++)
    {
      MMO_FS(pp->dpf[0], q, p7X_FS_C0) = vmulq_f32(MMO_FS(pp->dpf[0], q, p7X_FS_C0), sv);
      IMO_FS(pp->dpf[0], q)             = vmulq_f32(IMO_FS(pp->dpf[0], q),             sv);
    }
  XMXo(0, p7X_N) *= norm;
  XMXo(0, p7X_C) *= norm;
  XMXo(0, p7X_J) *= norm;
  ...
  for (x = 0; x < om_fs->abc->K; x++)
    {
      ...
      for (q = 0; q < Q; q++)
        {
          sv = vaddq_f32(sv, vmulq_f32(MMO_FS(pp->dpf[0], q, p7X_FS_C0), *rp)); rp++;
          sv = vaddq_f32(sv, IMO_FS(pp->dpf[0], q));  /* insert odds implicitly 1.0 */
        }
      esl_neon_hsum_float((esl_neon_128f_t){ .f32x4 = sv }, &(null2[x]));
      null2[x] += xfactor;
    }
 ...
 *
 * The generic path works in log space (using p7_FLogsum); the NEON path
  ...
  while (N--)
    {
      ...
      /* NEON path under test: forward -> backward -> decoding -> null2_fs */
...
#ifdef p7NULL2_FS_TESTDRIVE
/*
  gcc -g -Wall -march=armv8-a -std=gnu99 -o null2_fs_utest -I.. -L.. -I../../easel -L../../easel -Dp7NULL2_FS_TESTDRIVE null2_fs.c -lhmmer -leasel -lm
  ./null2_fs_utest
*/
#include "p7_config.h"
...
static char banner[] = "test driver for NEON frameshift null2 implementation";

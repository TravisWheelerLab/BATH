...
p7_fs_oprofile_Create(int allocM, const ESL_ALPHABET *abc, int codon_lengths)
{
  ...
  ESL_ALLOC(om_fs->rfv_mem, sizeof(float32x4_t) * nqf * ncodon_rows + 15);
  ESL_ALLOC(om_fs->tfv_mem, sizeof(float32x4_t) * nqf * p7O_NTRANS  + 15);

  /* pointer array: one float32x4_t * per emission row */
  ESL_ALLOC(om_fs->rfv, sizeof(float32x4_t *) * ncodon_rows);

  /* align vector memory on 16-byte boundaries */
  om_fs->rfv[0] = (float32x4_t *) (((unsigned long int) om_fs->rfv_mem + 15) & (~0xf));
  om_fs->tfv    = (float32x4_t *) (((unsigned long int) om_fs->tfv_mem + 15) & (~0xf));
 ...
 * stripes of 4 in the NEON float vectors.
 *
    ...
    for (k = 1, q = 0; q < nq; q++, k++)
      {
        ...
        om_fs->rfv[c][q] = esl_neon_expf((esl_neon_128f_t)tmp.v).f32x4;
      }
      ...
      for (t = p7O_BM; t <= p7O_II; t++)
        {
          ...
          om_fs->tfv[j++] = esl_neon_expf((esl_neon_128f_t)tmp.v).f32x4;
        }
    }
 ...
 * already holds log-odds scores, so we skip the esl_neon_expf / expf calls
 ...
 *            Viterbi implementation that uses vaddq_f32() rather than
 *            vmulq_f32(). It renders the profile unsuitable for
...
p7_fs_oprofile_Logify(P7_FS_OPROFILE *om_fs)
{
  ...
  for (c = 0; c < ncodon_rows; c++)
    for (j = 0; j < Q; j++)
      om_fs->rfv[c][j] = esl_neon_logf((esl_neon_128f_t)om_fs->rfv[c][j]).f32x4;

  /* Log-transform all float transition vectors (p7O_NTRANS * Q = 8*Q entries). */
  for (j = 0; j < p7O_NTRANS * Q; j++)
    om_fs->tfv[j] = esl_neon_logf((esl_neon_128f_t)om_fs->tfv[j]).f32x4;

/* P7_OIVX:
 * Vectorized intermediate-value matrix for the NEON spliced Viterbi.
 * Mirrors P7_IVX (p7_ivx.c) but stores float32x4_t vectors in striped
 ...
 *            float32x4_t vector for channel c and stripe q.  Backing memory
...
p7_oivx_Create(int M_hint, int C)
{
  ...
  ESL_ALLOC(ov->ivx_mem, sizeof(float32x4_t) * C * nq + 15);
  ESL_ALLOC(ov->ivx,     sizeof(float32x4_t *) * C);

  ov->ivx[0] = (float32x4_t *) (((unsigned long int) ov->ivx_mem + 15) & (~0xf));
  for (c = 1; c < C; c++)
...
p7_oivx_GrowTo(P7_OIVX *ov, int M, int C)
{
  ...
  if (C > ov->allocC) {
    ESL_RALLOC(ov->ivx, p, sizeof(float32x4_t *) * C);
  }

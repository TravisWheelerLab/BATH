/* NEON implementation of frameshift stochastic backtrace of a Forward matrix.
 ...
 * Synopsis:  Sample a traceback from a frameshift-aware Forward matrix; NEON version.
 *
 * Purpose:   Identical to <p7_GStochasticTrace_Frameshift()> except that
 *            <om_fs>, <ox> are NEON optimized versions of the frameshift
 ...
 * state type.
 *
 * Key differences from the standard NEON select_*() functions:
...
select_m_fs(ESL_RANDOMNESS *rng, const P7_FS_OPROFILE *om_fs, const P7_OMX *ox, int i, int k)
{
  ...
  float32x4_t *tp    = om_fs->tfv + 7*q;  /* transitions: BM, MM, IM, DM, MD, MI, II */
  float32x4_t  xBv   = vdupq_n_f32(ox->xmx[i*p7X_NXCELLS + p7X_B]);
  ...
  if (q > 0) {
    ...
    mpv = vextq_f32(zerov, MMO_FS(ox->dpf[i], Q-1, p7X_FS_C0), 3);
    dpv = vextq_f32(zerov, DMO_FS(ox->dpf[i], Q-1), 3);
    ipv = vextq_f32(zerov, IMO_FS(ox->dpf[i], Q-1), 3);
  }

  u.v = vmulq_f32(xBv, *tp); tp++;  path[0] = u.p[r];  /* B  * T_BM */
  u.v = vmulq_f32(mpv, *tp); tp++;  path[1] = u.p[r];  /* M' * T_MM */
  u.v = vmulq_f32(ipv, *tp); tp++;  path[2] = u.p[r];  /* I' * T_IM */
  u.v = vmulq_f32(dpv, *tp);        path[3] = u.p[r];  /* D' * T_DM */
...
select_d_fs(ESL_RANDOMNESS *rng, const P7_FS_OPROFILE *om_fs, const P7_OMX *ox, int i, int k)
{
  ...
  if (q > 0) {
    ...
    mpv  = vextq_f32(zerov, MMO_FS(ox->dpf[i], Q-1, p7X_FS_C0), 3);
    dpv  = vextq_f32(zerov, DMO_FS(ox->dpf[i], Q-1), 3);
    tmdv = vextq_f32(zerov, om_fs->tfv[7*(Q-1) + p7O_MD], 3);
    tddv = vextq_f32(zerov, om_fs->tfv[8*Q-1], 3);
  }
...
select_i_fs(ESL_RANDOMNESS *rng, const P7_FS_OPROFILE *om_fs, const P7_OMX *ox, int i, int k)
{
  ...
  int          q    = (k-1) % Q;
  int          r    = (k-1) / Q;
  float32x4_t  mpv  = MMO_FS(ox->dpf[i-3], q, p7X_FS_C0);
  float32x4_t  ipv  = IMO_FS(ox->dpf[i-3], q);
  float32x4_t *tp   = om_fs->tfv + 7*q + p7O_MI;
  union { float32x4_t v; float p[4]; } u;
  float   path[2];
  int     state[2] = { p7T_M, p7T_I };

  u.v = vmulq_f32(mpv, *tp); tp++;  path[0] = u.p[r];  /* M * T_MI */
  u.v = vmulq_f32(ipv, *tp);        path[1] = u.p[r];  /* I * T_II */
  esl_vec_FNorm(path, 2);
  return state[esl_rnd_FChoose(rng, path, 2)];
}
...
select_e_fs(ESL_RANDOMNESS *rng, const P7_FS_OPROFILE *om_fs, const P7_OMX *ox, int i, int *ret_k)
{
  ...
  float32x4_t  xEv  = vdupq_n_f32((float) norm);
  union { float32x4_t v; float p[4]; } u;
  int    q, r;
  ...
  while (1) {
    for (q = 0; q < Q; q++)
      {
        u.v = vmulq_f32(MMO_FS(ox->dpf[i], q, p7X_FS_C0), xEv);
        for (r = 0; r < 4; r++) {
          sum += u.p[r];
          if (roll < sum) { *ret_k = r*Q + q + 1; return p7T_M; }
        }

        u.v = vmulq_f32(DMO_FS(ox->dpf[i], q), xEv);
        for (r = 0; r < 4; r++) {
          sum += u.p[r];
...
/* utest_stotrace_fs()
 * Tests:
 *   1. Generic and NEON forward scores agree to tolerance.
 *   2. Each NEON stochastic trace is structurally valid: N >= 3, starts with
 *      S, ends with T.
 *   3. Both the generic and NEON stochastic trace functions complete without
  ...
  while (N--)
    {
      ...
      if (esl_FCompare_old(gfsc, ofsc, tolerance) != eslOK)
        esl_fatal("%s: generic fwd %.4f != NEON fwd %.4f (tol %.4f)", msg, gfsc, ofsc, tolerance);

      /* Sample NEON stochastic traces and validate each with p7_trace_fs_Validate */
      for (idx = 0; idx < 10; idx++)
        {
          char errbuf[eslERRBUFSIZE];
          if (p7_StochasticTrace_Frameshift(r, dsq, curr_L, om_fs5, fwd, tr) != eslOK)
            esl_fatal("%s: NEON stochastic trace failed", msg);
...
#ifdef p7STOTRACE_FS_TESTDRIVE
/*
  gcc -g -Wall -march=armv8-a -std=gnu99 -o stotrace_fs_utest -I.. -L.. -I../../easel -L../../easel -Dp7STOTRACE_FS_TESTDRIVE stotrace_fs.c -lhmmer -leasel -lm
  ./stotrace_fs_utest
*/
#include "p7_config.h"
...
static char banner[] = "test driver for NEON frameshift stochastic traceback";

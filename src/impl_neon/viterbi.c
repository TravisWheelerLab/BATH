/* Viterbi full matrix implementation; NEON version.
 *
...
/* Function:  p7_Viterbi()
 * Synopsis:  Full-matrix NEON Viterbi, enabling traceback.
 *
...
p7_Viterbi(const ESL_DSQ *dsq, int L, const P7_OPROFILE *om, P7_OMX *ox, float *ret_sc)
{
  ...
  float32x4_t *dpp;                      /* pointer to previous DP row ox->dpf[i-1]                  */
  float32x4_t *dp;                       /* pointer to current  DP row ox->dpf[i]                    */
  float32x4_t *rsc;                      /* will point at om->rfv[x] for residue x[i]                */
  float32x4_t *tsc;                      /* will point into (and step thru) om->tfv                  */
  ...
  for (i = 1; i <= L; i++)
    {
      ...
      mpv = vextq_f32(infv, MMO(dpp, Q-1), 3);
      dpv = vextq_f32(infv, DMO(dpp, Q-1), 3);
      ipv = vextq_f32(infv, IMO(dpp, Q-1), 3);

      for (q = 0; q < Q; q++)
	{
	  /* Calculate M(i,q); hold in sv (not yet stored). */
	  sv   =                vaddq_f32(xBv, *tsc);  tsc++;  /* B->Mk    */
	  sv   = vmaxq_f32(sv,  vaddq_f32(mpv, *tsc)); tsc++;  /* Mk-1->Mk */
	  sv   = vmaxq_f32(sv,  vaddq_f32(ipv, *tsc)); tsc++;  /* Ik-1->Mk */
	  sv   = vmaxq_f32(sv,  vaddq_f32(dpv, *tsc)); tsc++;  /* Dk-1->Mk */
	  sv   = vaddq_f32(sv, *rsc);                  rsc++;  /* + emission */
	  xEv  = vmaxq_f32(xEv, sv);                           /* Mk->E    */
	  ...
	  dcv = vaddq_f32(sv, *tsc); tsc++;  /* Mk->Dk+1 */

	  /* Calculate and store I(i,q). Insert emission = log(1) = 0 (background freq). */
	  sv        =                vaddq_f32(mpv, *tsc);  tsc++;  /* Mk->Ik   */
	  sv        = vmaxq_f32(sv,  vaddq_f32(ipv, *tsc)); tsc++;  /* Ik->Ik   */
	  IMO(dp,q) = sv;
	}
      ...
      for (q = 0; q < Q; q++)
	{
	  DMO(dp,q) = vmaxq_f32(dcv, DMO(dp,q));
	  dcv       = vaddq_f32(DMO(dp,q), *tsc); tsc++;
	}
	...
	for (q = 0; q < Q; q++)
	  {
	    if (! esl_neon_any_gt_float((esl_neon_128f_t){ .f32x4 = dcv },
                                       (esl_neon_128f_t){ .f32x4 = DMO(dp,q) })) break;
	    DMO(dp,q) = vmaxq_f32(dcv, DMO(dp,q));
	    dcv       = vaddq_f32(DMO(dp,q), *tsc); tsc++;
	  }
      } while (q == Q);
 ...
 * Synopsis:  Traceback from an lspace Viterbi DP matrix; NEON version.
 *
...
/*
   gcc -g -O3 -march=armv8-a -std=gnu99 -o viterbi_benchmark -I.. -L.. -I../../easel -L../../easel -Dp7VITERBI_BENCHMARK viterbi.c -lhmmer -leasel -lm
...
static char banner[] = "benchmark driver for NEON Viterbi";
...
{
  char        *msg = "NEON p7_Viterbi() unit test failed";
  P7_HMM      *hmm = NULL;
  P7_PROFILE  *gm  = NULL;
  P7_OPROFILE *om  = NULL;
  ESL_DSQ     *dsq = malloc(sizeof(ESL_DSQ) * (L+2));
  ...
  while (N--)
    {
      ...
      if (p7_trace_Validate(tr,  abc, dsq, errbuf) != eslOK) esl_fatal("%s: NEON trace invalid: %s",    msg, errbuf);
...
#ifdef p7VITERBI_TESTDRIVE
/*
   gcc -g -Wall -march=armv8-a -std=gnu99 -I.. -L.. -I../../easel -L../../easel -o viterbi_utest -Dp7VITERBI_TESTDRIVE viterbi.c -lhmmer -leasel -lm
   ./viterbi_utest
 */
#include "p7_config.h"
...
static char banner[] = "test driver for NEON p7_Viterbi() full-matrix implementation";

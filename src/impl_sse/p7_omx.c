...
/* Function:  p7_omx_DumpSP()
 * Synopsis:  Dump the full float SP DP matrix to a stream, for diagnostics.
 *
 * Purpose:   Dump the complete float (MDIP) DP matrix stored in <ox>
 *            to stream <fp>. Rows 0..L and model positions 1..M are
 *            printed in four sub-rows (M, I, D, P) per sequence position,
 *            together with the special state scores (E, N, J, B, C)
 *            stored in <ox->xmx>.
 *
 *            Uses the p7X_NSCELLS_SP=4 stride (MMO_SP/DMO_SP/IMO_SP/PMO_SP).
 *            Mirrors p7_gmx_DumpWindow() SP mode to facilitate comparison.
 *
 *            <ox> must be a full-matrix allocation (<allocL >= L>)
 *            with special-state storage (<ox->xmx != NULL>), as
 *            produced by <p7_omx_Create(M, L, L)>.
 *
 * Args:      fp      - output stream
 *            ox      - optimized full DP matrix (float, pspace, SP layout)
 *            istart  - first sequence row to print (0..L)
 *            iend    - last  sequence row to print (0..L)
 *            kstart  - first model position to print (0..M)
 *            kend    - last  model position to print (0..M)
 *            logify  - if TRUE, print log(value) instead of value
 *
 * Returns:   <eslOK> on success.
 *
 * Throws:    <eslEMEM> on allocation failure.
 */
int
p7_omx_DumpSP(FILE *fp, P7_OMX *ox, int istart, int iend, int kstart, int kend, int logify)
{
  int     M         = ox->M;
  int     Q         = p7O_NQF(M);
  int     width     = 9;
  ...
  /* Header */
  fprintf(fp, "      ");
  for (k = kstart; k <= kend; k++) fprintf(fp, "%*d ", width, k);
  fprintf(fp, "%*s %*s %*s %*s %*s\n", width, "E", width, "N", width, "J", width, "B", width, "C");
  fprintf(fp, "      ");
  for (k = kstart; k <= kend + 5; k++) fprintf(fp, "%*.*s ", width, width, "----------");
  fprintf(fp, "\n");

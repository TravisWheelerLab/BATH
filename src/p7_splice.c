...
p7_splice_ExtendPath(SPLICE_PIPELINE *pli, P7_TOPHITS *seed_hits, SPLICE_PATH *path, SPLICE_PATH *spliced_path, SPLICE_GRAPH *graph, SPLICE_BOUNDS *bounds)
{
  ...
  *************************/
  ...
  first_hit = th->hit[spliced_path->node_id[0]];
  ...
  /* Add any upstream seed nodes from the graph */
  down = spliced_path->node_id[0];
  for(up = graph->anchor_N; up < graph->num_nodes; up++) {
    if(!graph->node_in_graph[up]) continue;
    if(graph->tmp_node[up]) continue;
    if(p7_splicegraph_EdgeExists(graph, up, down)) {
      
      /* Dont Add if there is another anchor between the seed and the first hit */
      skip = FALSE;
      ...
      if(!skip) {
        p7_splicegraph_AddNode(tmp_graph, th->hit[up]);
        tmp_graph->orig_hit_idx[tmp_graph->num_nodes-1]  = up;
      }
    }
  }
  seeds_in_graph = tmp_graph->num_nodes;

  /* Add any upstream seed nodes from the seed hits */
  for(i = 0; i < seed_hits->N; i++) {
    ...
    if(p7_splice_HitUpstream(curr_hit->dcl, first_hit->dcl, tmp_graph->revcomp)) { 
      ...
      for(n = 0; n < graph->anchor_N; n++) {
        ...
        if(p7_splice_HitUpstream(curr_hit->dcl, th->hit[n]->dcl, tmp_graph->revcomp)) {
          if(p7_splice_HitBetween(curr_hit->dcl, th->hit[n]->dcl, first_hit->dcl, tmp_graph->revcomp)) { 
            skip = TRUE;
            break;
         }
        }
  ...
  ************************/
  ...
  last_hit  = th->hit[spliced_path->node_id[spliced_path->path_len-1]];
  ...
  /* Add any downstream seed nodes from the graph */
  up = spliced_path->node_id[spliced_path->path_len-1];
  for(down = graph->anchor_N; down < graph->num_nodes; down++) {
    if(!graph->node_in_graph[down]) continue;
    if(graph->tmp_node[down]) continue;
    if(p7_splicegraph_EdgeExists(graph, up, down)) {

      /* Dont Add if there is another anchor between the seed and the last hit */
      skip = FALSE;
      for(between = 0; between < graph->anchor_N; between++) {
        if(between == down) continue;
      ...
      if(!skip) {
        p7_splicegraph_AddNode(tmp_graph, th->hit[down]);
        tmp_graph->orig_hit_idx[tmp_graph->num_nodes-1] = down;
      }
    }
  }
  seeds_in_graph = tmp_graph->num_nodes;
  ...
  for(i = 0; i < seed_hits->N; i++) {
    ...
    if(p7_splice_HitUpstream(last_hit->dcl, curr_hit->dcl, tmp_graph->revcomp)) {
      ...
      for(n = 0; n < graph->anchor_N; n++) {
        if(!graph->node_in_graph[n]) continue;
        if(p7_splice_HitUpstream(th->hit[n]->dcl, curr_hit->dcl, tmp_graph->revcomp)) {
          if(p7_splice_HitBetween(last_hit->dcl, th->hit[n]->dcl, curr_hit->dcl, tmp_graph->revcomp)) {
            skip = TRUE;
            break;
          }
        }
...
p7_splice_SpliceExons(SPLICE_WORKER_INFO *info, SPLICE_PATH *orig_path, ESL_SQ *path_seq)
{
  ...
  /* Splice each pair or nodes*/  
  for(s = 1; s < orig_path->path_len; s++) {
...
p7_splice_AlignExons(SPLICE_WORKER_INFO *info, SPLICE_PATH *orig_path, ESL_SQ *path_seq, int down, int i_start, int i_end, int k_start, int k_end, int *next_i_start, int *next_k_start)
{
  ...
  p7_oivx_GrowTo(pli->acc_ov, M, SPLICE_ROWS);
  p7_oivx_GrowTo(pli->don_ov, M, SIGNAL_MEM_SIZE);

  p7_Viterbi_Spliced(path_seq->dsq, om_tr, ovit, pli->splice_scores->signal_scores, pli->acc_ov, pli->don_ov, i_start, i_end, pli->min_intron, TRUE, TRUE);
  ...
  p7_Viterbi_SplicedTrace(path_seq->dsq, ovit, gm_tr, pli->splice_scores->signal_scores, tr, i_start, i_end, k_start, k_end, pli->min_intron, &vitsc);
...
p7_splice_AlignExtendDown(SPLICE_WORKER_INFO *info, SPLICE_PATH *spliced_path, ESL_SQ *path_seq, int s_end, int i_start, int i_end, int k_start, int k_end, int *next_i_end, int *next_k_end)
{
  ...
  p7_Viterbi_Spliced(path_seq->dsq, om_tr, ovit, pli->splice_scores->signal_scores, pli->acc_ov, pli->don_ov, i_start, i_end, pli->min_intron, TRUE, FALSE);
  tr = p7_trace_fs_Create();
  p7_Viterbi_SplicedTrace(path_seq->dsq, ovit, gm_tr, pli->splice_scores->signal_scores, tr, i_start, i_end, k_start, k_end, pli->min_intron, &vitsc);
...
p7_splice_AlignExtendUp(SPLICE_WORKER_INFO *info, SPLICE_PATH *spliced_path, ESL_SQ *path_seq, int s_start, int i_start, int i_end, int k_start, int k_end)
{
  ...
  p7_Viterbi_Spliced(path_seq->dsq, om_tr, ovit, pli->splice_scores->signal_scores, pli->acc_ov, pli->don_ov, i_start, i_end, pli->min_intron, FALSE, TRUE);
  tr = p7_trace_fs_Create();
  p7_Viterbi_SplicedTrace(path_seq->dsq, ovit, gm_tr, pli->splice_scores->signal_scores, tr, i_start, i_end, k_start, k_end, pli->min_intron, &vitsc);
...
p7_splice_AlignSingle(SPLICE_WORKER_INFO *info, SPLICE_PATH *spliced_path, ESL_SQ *path_seq, int i_start, int i_end, int k_start, int k_end)
{
  ...
  p7_Viterbi_Spliced(path_seq->dsq, om_tr, ovit, pli->splice_scores->signal_scores, pli->acc_ov, pli->don_ov, i_start, i_end, pli->min_intron, TRUE, TRUE);
  tr = p7_trace_fs_Create();
  p7_Viterbi_SplicedTrace(path_seq->dsq, ovit, gm_tr, pli->splice_scores->signal_scores, tr, i_start, i_end, k_start, k_end, pli->min_intron, NULL);

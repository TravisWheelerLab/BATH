...
p7_splice_SpliceHits(P7_TOPHITS *tophits, SPLICE_SAVED_HITS *saved_hits, P7_HMM *hmm, P7_OPROFILE *om, P7_PROFILE *gm, P7_FS_PROFILE *gm_fs, P7_SCOREDATA *scoredata, ESL_GETOPTS *go, ESL_GENCODE *gcode, ESL_SQFILE *seq_file, FILE *ofp, int64_t db_nuc_cnt, int fs_pipe, int std_pipe)
{
  ...
  for ( i = 0; i < tophits->N; i++) {
    ...
    if(tophits->hit[i]->flags & p7_IS_REPORTED ) {
      seq_min = ESL_MIN(tophits->hit[i]->dcl->ienv, tophits->hit[i]->dcl->jenv);   
      seq_max = ESL_MAX(tophits->hit[i]->dcl->ienv, tophits->hit[i]->dcl->jenv);
      ali_seq = p7_splice_GetSubSequence(seq_file, tophits->hit[i]->name, seq_min, seq_max, (tophits->hit[i]->dcl->ienv < tophits->hit[i]->dcl->jenv ? 0 : 1));
...
hits_spliceable(P7_DOMAIN *upstream, P7_DOMAIN *downstream) {
  ...
  if(abs(downstream->iali - upstream->jali) > MAX_INTRON_LEN) return FALSE;
  ...
  for(i = first; i <= last; i++) {
    ...
    for(j = 0; j < graph->split_N; j++) {
     ...
     // p7_trans_VTrace(hit_seq->dsq, hit_seq->n, gcode, sub_fs_model, vit_mx, new_hit->dcl->tr);
      ...
      p7_splice_ComputeAliScores(new_hit->dcl, new_hit->dcl->tr, hit_aa_dsq, sub_model);      
    ...
    if(overlap_len > 0) { 
      ...
      if(i < graph->orig_N) {
        ...
        for(j = graph->orig_N; j < graph->split_N; j++) {
          ...
          if(hit_idx == split_idx && graph->in_graph[j]) {
            graph->in_graph[j] = FALSE;
            num_hits++;
          }
        }
        hits_processed[hit_idx] = TRUE;
...
p7_splice_ComputeAliScores_fs(P7_DOMAIN *dom, P7_TRACE *tr, ESL_DSQ *nuc_dsq, P7_FS_PROFILE *gm_fs, const ESL_ALPHABET *abc)
{
  ...
  while (z1<z2) {
    ...
    else if (tr->st[z1] == p7T_J) { // special case for merged split hits 
      dom->scores_per_pos[n] = -eslCONST_LOG2 + gm_fs->xsc[p7P_J][p7P_LOOP];
      z1++; n++;
      while (z1 < z2 && tr->st[z1] == p7T_J) {
        dom->scores_per_pos[n] = gm_fs->xsc[p7P_J][p7P_LOOP];
        z1++; n++;
      }
    }
    else ESL_XEXCEPTION(eslFAIL, "Impossible state from compute_ali_scores()");

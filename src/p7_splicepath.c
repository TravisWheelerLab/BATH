...
static SPLICE_PATH2* check_bypass_unspliced2(SPLICE_GRAPH *graph, SPLICE_PATH2 *path);
...
SPLICE_PATH2*
p7_splicepath_GetBestPath_Unspliced2(SPLICE_GRAPH *graph)
{
  ...
  /* Once a viable path has been found build the path */
  if ((path = p7_splicepath_Create2(path_len)) == NULL) goto ERROR;
  ...
  path = check_bypass_unspliced2(graph, path);

  return path;

  ERROR:
    if(path != NULL) p7_splicepath_Destroy2(path);
    return NULL; 
...
SPLICE_PATH2*
check_bypass_unspliced2(SPLICE_GRAPH *graph, SPLICE_PATH2 *path) 
{
  ...
  for(p = 0; p < path->path_len-1; p ++) {
    ...
    if(max_bypass_score > 0) {
      ...
      tmp_edge->bypass_checked = TRUE;
      p7_splicepath_Destroy2(path);
      path =  p7_splicepath_GetBestPath_Unspliced2(graph);    
      return path;
    }
  }
...
void
p7_splicepath_Dump2(FILE *fp, SPLICE_PATH2 *path)
{

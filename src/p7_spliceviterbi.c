...
p7_spliceviterbi_TranslatedGlobal(SPLICE_PIPELINE *pli, const ESL_DSQ *sub_dsq, const ESL_GENCODE *gcode, const P7_FS_PROFILE *gm_tr, P7_GMX *gx, int i_start, int i_end, int k_start, int k_end)
{
  ...
  /*Special cases for the rows 3-min_intron+2 (no donor site lookback)*/
  ...
  for (i = 3; i <= loop_end; i++)
  {
    ...
    MMX_SP(i,1) = XMX_SP(i-3,p7G_B) + p7P_MSC_CODON(gm_tr, sub_k, c3);

    if(p7P_MSC_CODON(gm_tr, sub_k, c3) == -eslINFINITY)
    ...
    for (k = 2; k < M; k++) {
      ...
      MMX_SP(i,k) = ESL_MAX(MMX_SP(i-3,k-1) + TSC(p7P_MM,sub_k-1),
                    ESL_MAX(IMX_SP(i-3,k-1) + TSC(p7P_IM,sub_k-1),
                            DMX_SP(i-3,k-1) + TSC(p7P_DM,sub_k-1))) + p7P_MSC_CODON(gm_tr, sub_k, c3);
  ...
  for (i = min_intron+3; i <= L; i++)
  {
    ...
    if(SIGNAL(sub_dsq[sub_i-min_intron], sub_dsq[sub_i-min_intron+1]) == DONOR_GT) XXGT = TRUE;
    else                         XXGT = FALSE;

    if(SIGNAL(sub_dsq[sub_i-min_intron], sub_dsq[sub_i-min_intron+1]) == DONOR_GC) XXGC = TRUE;
    else                         XXGC = FALSE;

    if(SIGNAL(sub_dsq[sub_i-min_intron], sub_dsq[sub_i-min_intron+1]) == DONOR_AT) XXAT = TRUE;
    else                         XXAT = FALSE;
    ...
    for (k = 1; k < M; k++) {
      ...
      MMX_SP(i,k) = ESL_MAX(MMX_SP(i-3,k-1) + TSC(p7P_MM,sub_k-1),
                    ...
                    ESL_MAX(DMX_SP(i-3,k-1) + TSC(p7P_DM,sub_k-1),
                            PMX_SP(i-3,k-1) + TSC_P))) + p7P_MSC_CODON(gm_tr, sub_k, c3);
      
      if(p7P_MSC_CODON(gm_tr, sub_k, c3) == -eslINFINITY) IMX_SP(i,k) = -eslINFINITY;
    ...
    if(AGXXX) {
      for (k = 2; k < M; k++) {
        sub_k = k_start + k -1;
        TMP_SC = SSX0(k, p7S_GTAG) + signal_scores[p7S_GTAG] + p7P_MSC_CODON(gm_tr, sub_k, c3);
        if(TMP_SC > PMX_SP(i,k)) {
            PMX_SP(i,k) = TMP_SC;
            lookback[i][k] = SIX0(k, p7S_GTAG);
        }

        TMP_SC = SSX0(k, p7S_GCAG) + signal_scores[p7S_GCAG] + p7P_MSC_CODON(gm_tr, sub_k, c3);
        if(TMP_SC > PMX_SP(i,k)) {
            PMX_SP(i,k) = TMP_SC;
      ...
      for (k = 2; k < M; k++) {
        sub_k = k_start + k -1;
        TMP_SC = SSX0(k, p7S_ATAC) + signal_scores[p7S_ATAC] + p7P_MSC_CODON(gm_tr, sub_k, c3);
        if(TMP_SC > PMX_SP(i,k)) {
            PMX_SP(i,k) = TMP_SC;
    ...
    if(AGXX) {
      for(nuc1 = 0; nuc1 < 4; nuc1++) {
        ...
        for (k = 2; k < M; k++) {
          ...
          TMP_SC = SSX1(k, p7S_GCAG, nuc1) + signal_scores[p7S_GCAG] + p7P_MSC_CODON(gm_tr, sub_k, c2);
          if(TMP_SC > PMX_SP(i,k)) {
            PMX_SP(i,k) = TMP_SC;
      ...
      for(nuc1 = 0; nuc1 < 4; nuc1++) {
        for(nuc2 = 0; nuc2 < 4; nuc2++) {
          ...
          for (k = 2; k < M; k++) {
            ...
            TMP_SC = SSX2(k, p7S_GCAG, nuc1, nuc2) + signal_scores[p7S_GCAG] + p7P_MSC_CODON(gm_tr, sub_k, c1);
            if(TMP_SC > PMX_SP(i,k)) {
              PMX_SP(i,k) = TMP_SC;
    ...
    if(XXGT && t < MAX_NUC && u < MAX_NUC)  {
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX2(k, p7S_GTAG, t, u)) {
          ...
          SIX2(k, p7S_GTAG, t, u) = i-min_intron+1;
        }
      }
    } 
      ...
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX2(k, p7S_GCAG, t, u)) {
          ...
          SIX2(k, p7S_GCAG, t, u) = i-min_intron+1;
        }
      }
    }
      ...
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX2(k, p7S_ATAC, t, u)) {
          ...
          SIX2(k, p7S_ATAC, t, u) = i-min_intron+1;
        }
      }
    }
    ...
    if(XGT && t < MAX_NUC)  {
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX1(k, p7S_GTAG, t)) {
          ...
          SIX1(k, p7S_GTAG, t) = i-min_intron;
        }
      }
    }
      ...
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX1(k, p7S_GCAG, t)) {
          ...
          SIX1(k, p7S_GCAG, t) = i-min_intron;
        }
      }
    }
      ...
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX1(k, p7S_ATAC, t)) {
          ...
          SIX1(k, p7S_ATAC, t) = i-min_intron;
        }
      }
    }
    ...
    if(GT) {
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX0(k, p7S_GTAG)) {
          ...
          SIX0(k, p7S_GTAG) = i-min_intron-1;
        }
      }
    }
    else if(GC) {
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX0(k, p7S_GCAG)) {
          ...
          SIX0(k, p7S_GCAG) = i-min_intron-1;
        }
      }
    }
    else if(AT) {
      for (k = 2; k < M; k++) {
        ...
        if(TMP_SC > SSX0(k, p7S_ATAC)) {
          ...
          SIX0(k, p7S_ATAC) = i-min_intron-1;
        }
      }
    }
...
p7_spliceviterbi_TranslatedSemiGlobalExtendDown(SPLICE_PIPELINE *pli, const ESL_DSQ *sub_dsq, const ESL_GENCODE *gcode, const P7_FS_PROFILE *gm_tr, P7_GMX *gx, int i_start, int i_end, int k_start, int k_end)
{
  ...
  /*Special cases for the rows 3-min_intron+2*/
  AGXXX = ACXXX = AGXX = ACXX = AGX = ACX  = FALSE;
  XXGT = XXGC = XXAT = XGT = XGC = XAT = GT = GC = AT = FALSE;
  loop_end = ESL_MIN(L, min_intron+2);
  for (i = 3; i <= loop_end; i++)

...
p7_tophits_Domains(FILE *ofp, P7_TOPHITS *th, P7_PIPELINE *pli, int textw)
{
  ...
  for (h = 0; h < th->N; h++) {
    if (th->hit[h]->flags & p7_IS_REPORTED)
    {
      ...
      {
        /* 
          score  bias    Evalue hmmfrom  hmm to     alifrom    ali to      exons    sq len      acc
          ------ ----- --------- ------- -------    --------- ---------    -----    ---------    ----
        */
        
        if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %5s  %9s   %4s\n",
          "score",  "bias",   "   Evalue", "hmm-from", " hmm-to", " ali-from", "   ali-to", "exons", "   sq-len", "acc")  < 0)
          ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
      ...
      {
        /* 
           score  bias    Evalue hmmfrom  hmm to     alifrom    ali to      shifts     stops   sq len      acc
           ------ ----- --------- ------- -------    --------- ---------    ------     ----- ---------    ----
        */

          if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %6s  %5s %9s   %4s\n",  
           "score",  "bias",   "   Evalue", "hmm-from", " hmm-to", " ali-from", "   ali-to", "shifts", "stops", "   sq-len", "acc")  < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
        ...
        /*
           score  bias    Evalue hmmfrom  hmm to     alifrom    ali to      sq len      acc
           ------ ----- --------- ------- -------    --------- ---------    ---------    ----
        */
  
        if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %9s   %4s\n",
        "score",  "bias",   "   Evalue", "hmm-from", " hmm-to", " ali-from", "   ali-to", "   sq-len", "acc")  < 0)
          ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
      ...
      if (pli->spliced)
      {
        if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %5d  %9" PRId64 "   %4.2f\n",
      ...
      {
        if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %6d  %5d %9" PRId64 "   %4.2f\n",
            th->hit[h]->dcl[0].is_included ? '!' : '?',
            th->hit[h]->dcl[0].bitscore,
            th->hit[h]->dcl[0].dombias * eslCONST_LOG2R, /* convert NATS to BITS at last moment */
            exp(th->hit[h]->dcl[0].lnP), // * pli->Z,
	  ...
	  {
        if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %9" PRId64 "   %4.2f\n",
            th->hit[h]->dcl[0].is_included ? '!' : '?',
            th->hit[h]->dcl[0].bitscore,
            th->hit[h]->dcl[0].dombias * eslCONST_LOG2R, /* convert NATS to BITS at last moment */
            exp(th->hit[h]->dcl[0].lnP), // * pli->Z,
	  ...
	  if (pli->show_alignments)
 	  {
		...
		if ((status = p7_alidisplay_Print_BATH(ofp, th->hit[h]->dcl[0].ad, 30, 40, textw, pli)) != eslOK) return status;
...
p7_tophits_TabularTargets(FILE *ofp, char *qname, char *qacc, P7_TOPHITS *th, P7_PIPELINE *pli, int show_header)
{
  ...
  if (show_header)
  {
    ...
    if (fprintf(ofp, "  %9s %6s %5s %5s", "  E-value", " score", " bias", "  PID") < 0)
      ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
  ...
  for (h = 0; h < th->N; h++)
  {
    if (th->hit[h]->flags & p7_IS_REPORTED)    
    {
      ...
      if (fprintf(ofp, " %9.2g %6.1f %5.1f %5.2f",
        exp(th->hit[h]->lnP), 
        th->hit[h]->score,
        th->hit[h]->dcl[0].dombias * eslCONST_LOG2R,
        th->hit[h]->dcl[0].ad->pid) < 0) /* convert NATS to BITS at last moment */
          ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
 ...
 */
int
p7_tophits_TabularExons(FILE *ofp, char *qname, char *qacc, P7_TOPHITS *th, P7_PIPELINE *pli, int show_header, int node_info)
{
  ...
  if (show_header)
  {
    ...
    if (fprintf(ofp, "#%7s %-*s %-*s %-*s %-*s %9s %9s %9s %6s %5s %3s %3s %9s %9s %9s %9s %9s %5s %7s",
            " hit ID", tnamew, " target name",        taccw, " accession", qnamew, " query name",          qaccw, " accession", "  hmm len", "  seq len", "  E-value", " score", " bias", "  #", " of", " hmm from", "   hmm to", " ali from", "   ali to", "  P-value", "  PID", " splice") < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");

    if(node_info) {
      if (fprintf(ofp, " %7s %7s\n", " anchor", " extend") < 0) ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed"); 
    }
    else {
      if (fprintf(ofp, "\n")  < 0) ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");
    }
  ...
  for (h = 0; h < th->N; h++)
    if (th->hit[h]->flags & p7_IS_REPORTED) {
      ...
      for(x = 0; x < th->hit[h]->dcl->ad->exon_cnt; x++) {
                ...
                if (fprintf(ofp, "%8d %-*s %-*s %-*s %-*s %9d %*" PRId64 " %9.2g %6.1f %5.1f %3d %3d ",
                id,
                tnamew, th->hit[h]->name,
                taccw,  th->hit[h]->acc ? th->hit[h]->acc : "-",
                qnamew, qname,
        ...
        if(th->hit[h]->dcl->ad->exon_cnt > 1) {
          if (fprintf(ofp, "%9d %9d %*" PRId64 " %*" PRId64 " %9.2g %5.2f ",
          ...
          if     (th->hit[h]->dcl->ad->exon_splice[x] == p7S_GTAG) { if (fprintf(ofp, "%7s ", "gtag") < 0) ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed"); } 
          else if(th->hit[h]->dcl->ad->exon_splice[x] == p7S_GCAG) { if (fprintf(ofp, "%7s ", "gcag") < 0) ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed"); } 
          else if(th->hit[h]->dcl->ad->exon_splice[x] == p7S_ATAC) { if (fprintf(ofp, "%7s ", "atac") < 0) ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed"); }
          else                                                     { if (fprintf(ofp, "%7s ", "----") < 0) ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed"); }

        }
        else {

          if (fprintf(ofp, "%9d %9d %*" PRId64 " %*" PRId64 " %9.2g %5.2f %7s ",
                th->hit[h]->dcl->ad->hmmfrom,
                th->hit[h]->dcl->ad->hmmto,
                posw, th->hit[h]->dcl->ad->sqfrom,
                posw, th->hit[h]->dcl->ad->sqto,
                exp(th->hit[h]->sum_lnP),
                th->hit[h]->dcl->ad->pid,
                "----") < 0)
                ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");
        ...
        if(node_info) {
          if (fprintf(ofp, "%7s %7s\n",
                  ((th->hit[h]->dcl->ad->exon_cnt == 1 || th->hit[h]->dcl->ad->exon_anchor[x]) ? "True" : "False"),
                  ((th->hit[h]->dcl->ad->exon_cnt  > 1 && th->hit[h]->dcl->ad->exon_extend[x]) ? "True" : "False")) < 0)
                  ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");    
        }
        else {
          if (fprintf(ofp, "\n") < 0) ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");

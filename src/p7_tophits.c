...
p7_tophits_Targets(FILE *ofp, P7_TOPHITS *th, P7_PIPELINE *pli, int textw)
{
  ...
  if (pli->spliced)
  {
    ...
    /*  E-value  score  bias  Sequence     start    end  exons  Description */
    if (fprintf(ofp, "  %9s %6s %5s  %-*s %*s %*s  %5s  %s\n",
      "E-value", " score", " bias", namew, (pli->mode == p7_SEARCH_SEQS ? "Sequence":"Model"), posw, "start", posw, "end", "exons", "Description") < 0)
      ESL_EXCEPTION_SYS(eslEWRITE, "per-sequence hit list: write failed");
  ...
  {	
    if (th->hit[h]->flags & p7_IS_REPORTED)
    {
      ...
      if (pli->spliced)
      {
        if (fprintf(ofp, "%c %9.2g %6.1f %5.1f  %-*s %*" PRId64 " %*" PRId64 "  %5d",
        newness,
...
p7_tophits_Domains(FILE *ofp, P7_TOPHITS *th, P7_PIPELINE *pli, int textw)
{
  ...
  for (h = 0; h < th->N; h++) {
    if (th->hit[h]->flags & p7_IS_REPORTED)
    {
      ...
      {
        /* 
          score  bias    Evalue hmmfrom  hmm to     alifrom    ali to      envfrom    env to   exons    sq len      acc
          ------ ----- --------- ------- -------    --------- ---------    --------- --------- -----    ---------    ----
        */
        
        if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %9s %9s    %5s  %9s   %4s\n",
          "score",  "bias",   "   Evalue", "hmm-from", " hmm-to", " ali-from", "   ali-to", " env-from", "   env-to", "exons", "   sq-len", "acc")  < 0)
          ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
      ...
      else if(pli->fs_pipe) 
      {
        ...
        */
        ...
        if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %9s %9s    %9s   %4s\n",
        "score",  "bias",   "   Evalue", "hmm-from", " hmm-to", " ali-from", "   ali-to", " env-from", "   env-to", "   sq-len", "acc")  < 0)
          ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
      ...
      if (pli->spliced)
      {
        if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %9" PRId64 " %9" PRId64 " %c%c %5d  %9" PRId64 "   %4.2f\n",
            th->hit[h]->dcl[0].is_included ? '!' : '?',
            th->hit[h]->dcl[0].bitscore,
            th->hit[h]->dcl[0].dombias * eslCONST_LOG2R, /* convert NATS to BITS at last moment */
            exp(th->hit[h]->dcl[0].lnP), // * pli->Z,
            th->hit[h]->dcl[0].ad->hmmfrom,
            th->hit[h]->dcl[0].ad->hmmto,
            (th->hit[h]->dcl[0].ad->hmmfrom == 1) ? '[' : '.',
            (th->hit[h]->dcl[0].ad->hmmto   == th->hit[h]->dcl[0].ad->M) ? ']' : '.',
            th->hit[h]->dcl[0].ad->sqfrom,
            th->hit[h]->dcl[0].ad->sqto,
            (th->hit[h]->dcl[0].ad->sqfrom == 1) ? '[' : '.',
            (th->hit[h]->dcl[0].ad->sqto   == th->hit[h]->dcl[0].ad->L) ? ']' : '.',
            th->hit[h]->dcl[0].ienv,
            th->hit[h]->dcl[0].jenv,
            (th->hit[h]->dcl[0].ienv == 1) ? '[' : '.',
            (th->hit[h]->dcl[0].jenv == th->hit[h]->dcl[0].ad->L) ? ']' : '.',
            th->hit[h]->dcl[0].ad->exon_cnt,
            th->hit[h]->dcl[0].ad->L,
            (th->hit[h]->dcl[0].oasc / (1.0 + fabs((float) (th->hit[h]->dcl[0].jenv - th->hit[h]->dcl[0].ienv) / 3.0)))) < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
      }
      else if(pli->fs_pipe) 
      {
        if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %9" PRId64 " %9" PRId64 " %c%c %6d  %5d %9" PRId64 "   %4.2f\n",
            ...
            (th->hit[h]->dcl[0].ienv == 1) ? '[' : '.',
            (th->hit[h]->dcl[0].jenv == th->hit[h]->dcl[0].ad->L) ? ']' : '.',
            th->hit[h]->dcl[0].ad->frameshifts,
            th->hit[h]->dcl[0].ad->stops,
            th->hit[h]->dcl[0].ad->L,
            (th->hit[h]->dcl[0].oasc / (1.0 + fabs((float) (th->hit[h]->dcl[0].jenv - th->hit[h]->dcl[0].ienv) / 3)))) < 0) 
            ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
      }
      else
	  {
        if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %9" PRId64 " %9" PRId64 " %c%c %9" PRId64 "   %4.2f\n",
            th->hit[h]->dcl[0].is_included ? '!' : '?',
            th->hit[h]->dcl[0].bitscore,
            th->hit[h]->dcl[0].dombias * eslCONST_LOG2R, /* convert NATS to BITS at last moment */
            exp(th->hit[h]->dcl[0].lnP), // * pli->Z,
	  ...
	  /* Alignment data for each reported hit. */
	  if (pli->show_alignments)
 	  {
	    if (fprintf(ofp, "\n  Alignment:\n") < 0)
	      ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");

		if (fprintf(ofp, "  score: %.1f bits", th->hit[h]->dcl[0].bitscore) < 0)
		  ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
		  
		if (fprintf(ofp, "\n") < 0)
	      ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");

		if ((status = p7_alidisplay_Print(ofp, th->hit[h]->dcl[0].ad, 40, 40, textw, pli)) != eslOK) return status;
...
p7_tophits_TabularTargets(FILE *ofp, char *qname, char *qacc, P7_TOPHITS *th, P7_PIPELINE *pli, int show_header)
{
  ...
  if (show_header)
  {
    ...
    if (fprintf(ofp, "#%7s %-*s %-*s %-*s %-*s %9s %9s %9s %9s %9s %9s",
        " hit ID", tnamew-1, " target name",        taccw, " accession",  qnamew, " query name",          qaccw, " accession",  "  hmm len",  " hmm from", "   hmm to", "  seq len", " ali from", "   ali to") < 0)
         ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");

    if(pli->spliced) {
      if (fprintf(ofp, " %9s", " exon cnt") < 0)
        ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
    }
    else {
      if (fprintf(ofp, " %9s %9s", " env from", "   env to") < 0)
        ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
    }

    if (fprintf(ofp, "  %9s %6s %5s", "  E-value", " score", " bias") < 0)
      ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");

    if(pli->pid) { 
      if (fprintf(ofp, " %6s", "   pid") < 0)
        ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
    }          

    if(pli->fs_pipe) {
      if (fprintf(ofp, " %7s %6s", " shifts", " stops") < 0)
        ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
    }

    if (fprintf(ofp, " %s\n", (pli->show_cigar ? "CIGAR" : " description of target")) < 0)
      ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
    ...
    if(pli->spliced) {
      if (fprintf(ofp, " %9s", "---------") < 0)
       ...
       if (fprintf(ofp, " %9s %9s", "---------", "---------") < 0)
        ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
    }

    if (fprintf(ofp, "  %9s %6s %5s", "---------", "------", "-----") < 0)
      ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
  ...
  for (h = 0; h < th->N; h++)
  {
    if (th->hit[h]->flags & p7_IS_REPORTED)    
    {
      ...
      if (fprintf(ofp, "%8d %-*s %-*s %-*s %-*s %8d  %8d  %8d  %*" PRId64 " %*" PRId64 " %*" PRId64 "",
            id,
            tnamew, th->hit[h]->name,
            taccw,  th->hit[h]->acc ? th->hit[h]->acc : "-",
            qnamew, qname,
            ...
            posw, th->hit[h]->dcl[0].jali) < 0)
	        ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");

      if(pli->spliced) {
                  
        if (fprintf(ofp, " %8d ",
            th->hit[h]->dcl[0].ad->exon_cnt) < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
      }
      else {
        
         if (fprintf(ofp, " %*" PRId64 " %*" PRId64 "", 
            posw, th->hit[h]->dcl[0].ienv,
            posw, th->hit[h]->dcl[0].jenv) < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
      }            

	  if (fprintf(ofp, " %9.2g %6.1f %5.1f",
            exp(th->hit[h]->lnP), 
            th->hit[h]->score,
            th->hit[h]->dcl[0].dombias * eslCONST_LOG2R) < 0) /* convert NATS to BITS at last moment */
		        ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");

  	  if(pli->pid) {
        if (fprintf(ofp, " %6.2f ", 
            th->hit[h]->dcl[0].ad->pid) < 0) 
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
      }

      if(pli->fs_pipe) {
        if (fprintf(ofp, " %7d %6d",
            th->hit[h]->dcl[0].ad->frameshifts,
            th->hit[h]->dcl[0].ad->stops) < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-sequence hit list: write failed");
      }
...
p7_tophits_TabularDomains(FILE *ofp, char *qname, char *qacc, P7_TOPHITS *th, P7_PIPELINE *pli, int show_header)
{
  ...
  if (show_header)
  {
      if(pli->frameshift)
      {
        if (fprintf(ofp, "#%*s %33s %29s %10s %18s %18s\n", tnamew+qnamew-1+15+taccw+qaccw, "",                             "--- full sequence ---",        "---- this domain ----",                   "hmm coord",                    "ali coord",                "env coord") < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-domain hit list: write failed");
         if (fprintf(ofp, "#%-*s %-*s %15s %-*s %-*s %5s %9s %6s %5s %3s %3s %9s %6s %5s %5s %5s %9s %9s %9s %9s %5s %12s %6s %9s\n",
            tnamew-1, " target name",        taccw, "accession",  "tlen",  qnamew, "query name",           qaccw, "accession",  "qlen",  "E-value",   "score",  "bias",  "#",   "of",  "i-Evalue",  "score",  "bias",  "from",  "to",    "from",        "to",      "from",        "to",   "acc",  "frameshifts", "stops", "pipeline") < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-domain hit list: write failed");                                                                                                                                               
         if (fprintf(ofp, "#%*s %*s %15s %*s %*s %5s %9s %6s %5s %3s %3s %9s %6s %5s %5s %5s %9s %9s %9s %9s %5s %12s %6s %9s\n",                                                                                             
           tnamew-1, "-------------------", taccw, "----------", "---------------", qnamew, "--------------------", qaccw, "----------", "-----", "---------", "------", "-----", "---", "---",  "---------", "------", "-----", "-----", "-----", "---------", "---------", "---------", "---------", "-----", "------------", "------", "---------" ) < 0)
           ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-domain hit list: write failed");
      }
      else if (th->N > 0 && th->hit[0]->ndom > 0 && th->hit[0]->dcl[0].ad->ntseq != NULL)
  ...
  for (h = 0; h < th->N; h++)
    if (th->hit[h]->flags & p7_IS_REPORTED) {
        ...
        for (d = 0; d < th->hit[h]->ndom; d++)
	      ...
	      if(pli->frameshift)
	      {
                if (fprintf(ofp, "%-*s %-*s %15" PRId64 " %-*s %-*s %5d %9.2g %6.1f %5.1f %3d %3d %9.2g %6.1f %5.1f %23d %5d %9" PRId64 " %9" PRId64 " %9" PRId64 " %9" PRId64 " %5.2f %12d %6d %s\n",
                tnamew, th->hit[h]->name,
                taccw,  th->hit[h]->acc ? th->hit[h]->acc : "-",
                th->hit[h]->dcl[0].ad->L,
                qnamew, qname,
		...
		exp(th->hit[h]->lnP), // * pli->Z,
                ...
                th->hit[h]->dcl[d].ienv,
                th->hit[h]->dcl[d].jenv,
                (th->hit[h]->dcl[d].oasc / (1.0 + fabs((th->hit[h]->dcl[d].jenv - th->hit[h]->dcl[d].ienv) / 3.0))),
                th->hit[h]->dcl[d].ad->frameshifts,
		th->hit[h]->dcl[d].ad->stops,
		th->hit[h]->frameshift ? "fs" : "std" ) < 0)
                  ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-domain hit list: write failed");
	      }
	      else if (th->hit[h]->dcl[d].ad->ntseq != NULL)
              {
...
p7_tophits_TabularExons(FILE *ofp, char *qname, char *qacc, P7_TOPHITS *th, P7_PIPELINE *pli, int show_header)
{
  ...
  if (show_header)
  {
    ...
    if(pli->pid) {
      ...
      if (fprintf(ofp, "#%7s %-*s %-*s %-*s %-*s %9s %9s %9s %6s %5s %7s %6s %3s %3s %9s %9s %9s %9s %9s %6s %5s %5s %6s %6s\n",
            " hit ID", tnamew, " target name",        taccw, " accession", qnamew, " query name",          qaccw, " accession", "  hmm len", "  seq len", "  E-value", " score", " bias", " shifts", " stops", "  #", " of", " hmm from", "   hmm to", " ali from", "   ali to", "  E-value", " score", " bias", "  PID", " orig", "split") < 0)    
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");
...
//TODO realigned the "this exon" portion
      if (fprintf(ofp, "#%*s %37s %47s \n", tnamew+qnamew+31+taccw+qaccw, "",                                                                      "------------- full hit ------------- ", "-------------------------------- this exon --------------------------------") < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");

      if (fprintf(ofp, "#%7s %-*s %-*s %-*s %-*s %9s %9s %9s %6s %5s %7s %6s %3s %3s %9s %9s %9s %9s %9s %6s %5s %6s %6s\n",
            " hit ID", tnamew, " target name",        taccw, " accession", qnamew, " query name",          qaccw, " accession", "  hmm len", "  seq len", "  E-value", " score", " bias", " shifts", " stops", "  #", " of", " hmm from", "   hmm to", " ali from", "   ali to", "  E-value", " score", " bias", " orig", "split") < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");
  ...
  for (h = 0; h < th->N; h++)
    if (th->hit[h]->flags & p7_IS_REPORTED) {
      ...
      for(x = 0; x < th->hit[h]->dcl->ad->exon_cnt; x++) {
        ...
        if (fprintf(ofp, "%8d %-*s %-*s %-*s %-*s %9d %*" PRId64 " %9.2g %6.1f %5.1f %7d %6d %3d %3d ",
                id,
                tnamew, th->hit[h]->name,
                taccw,  th->hit[h]->acc ? th->hit[h]->acc : "-",
                qnamew, qname,
                ...
                (x+1),
                th->hit[h]->dcl->ad->exon_cnt) < 0)
                ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");

        if(th->hit[h]->dcl->ad->exon_cnt > 1) {
          if (fprintf(ofp, "%9d %9d %*" PRId64 " %*" PRId64 " %9.2g %6.1f %5.1f ",
                th->hit[h]->dcl->ad->exon_hmm_starts[x],
                th->hit[h]->dcl->ad->exon_hmm_ends[x],
                posw, th->hit[h]->dcl->ad->exon_seq_starts[x],
                posw, th->hit[h]->dcl->ad->exon_seq_ends[x],
                exp(th->hit[h]->dcl->ad->exon_lnP[x]),       
                th->hit[h]->dcl->ad->exon_score[x],
                th->hit[h]->dcl->ad->exon_bias[x]) < 0)
                ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");
          ...
          if (fprintf(ofp, "%9d %9d %*" PRId64 " %*" PRId64 " %9.2g %6.1f %5.1f ",
                th->hit[h]->dcl->ad->hmmfrom,
                th->hit[h]->dcl->ad->hmmto,
                posw, th->hit[h]->dcl->ad->sqfrom,
                posw, th->hit[h]->dcl->ad->sqto,
                exp(th->hit[h]->lnP),
                th->hit[h]->score,
                th->hit[h]->dcl->dombias * eslCONST_LOG2R) < 0)
                ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");
...
//TODO remove these output fields 
        if (fprintf(ofp, "%6s %6s\n",
                ((th->hit[h]->dcl->ad->exon_cnt == 1 || th->hit[h]->dcl->ad->exon_orig[x])  ? "True" : "False"),
                ((th->hit[h]->dcl->ad->exon_cnt  > 1 && th->hit[h]->dcl->ad->exon_split[x]) ? "True" : "False")) < 0)
                ESL_EXCEPTION_SYS(eslEWRITE, "tabular per-exon hit list: write failed");    
      }
    }
  return eslOK;

...
p7_tophits_Threshold(P7_TOPHITS *th, P7_PIPELINE *pli)
{
  ...
  {  
    for (h = 0; h < th->N; h++)
    {
      if ( !(th->hit[h]->flags & p7_IS_DUPLICATE) && p7_pli_TargetReportable(pli, th->hit[h]->score, th->hit[h]->lnP))
      {
          ...
          if (p7_pli_TargetIncludable(pli, th->hit[h]->score, th->hit[h]->lnP))
	   {
          ...
          if (pli->frameshift) { // no domains in dna search, so:
...
p7_tophits_Targets(FILE *ofp, P7_TOPHITS *th, P7_PIPELINE *pli, int textw)
{
  ...
  if (pli->frameshift) 
  {
      ...
      if (fprintf(ofp, "  %9s %6s %5s  %-*s %*s %*s  ",
      "E-value", " score", " bias", namew, (pli->mode == p7_SEARCH_SEQS ? "Sequence":"Model"), posw, "start", posw, "end") < 0)
        ESL_EXCEPTION_SYS(eslEWRITE, "per-sequence hit list: write failed");

      if(pli->fs_pipe) {
        if (fprintf(ofp, "%6s  %5s  ", "shifts", "stops") < 0)
          ESL_EXCEPTION_SYS(eslEWRITE, "per-sequence hit list: write failed");
      }
  ...
  {	
    if (th->hit[h]->flags & p7_IS_REPORTED)
    {
      ...
      if (pli->spliced)
      {
        if (fprintf(ofp, "%c %9.2g %6.1f %5.1f  %-*s %*" PRId64 " %*" PRId64 "  %8d  %6d  %5d",
        newness,
...
p7_tophits_Domains(FILE *ofp, P7_TOPHITS *th, P7_PIPELINE *pli, int textw)
{
  ...
  { 
    if (fprintf(ofp, "Domain annotation for each %s%s:\n", pli->mode == p7_SEARCH_SEQS ? "sequence" : "model", pli->show_alignments ? " (and alignments)" : "") < 0)
        ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
  }
    ...
    if (th->hit[h]->flags & p7_IS_REPORTED)
    {
      ...
      if(pli->spliced)
      {
        /* The dna hit table is 119 char wide
          score  bias    Evalue hmmfrom  hmm to     alifrom    ali to      envfrom    env to   exon-cnt    shifts     stops   sq len      acc
          ------ ----- --------- ------- -------    --------- ---------    --------- --------- --------    ------     ----- ---------    ----
        */
        
        if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %9s %9s    %8s  %6s  %5s %9s   %4s\n",
          "score",  "bias",   "   Evalue", "hmm-from", " hmm-to", " ali-from", "   ali-to", " env-from", "   env-to", "exon-cnt", "shifts", "stops", "   sq-len", "acc")  < 0)
          ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
        if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %9s %9s    %8s  %6s  %5s %9s   %4s\n",
          "------", "-----", "---------", "--------", "-------", "---------", "---------", "---------", "---------", "--------", "------",  "-----", "---------", "----") < 0)
          ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
      ...
      else if(pli->frameshift) 
      {
        ...
        */
          ...
          if (fprintf(ofp, "   %6s %5s %9s %10s %9s    %9s %9s    %9s %9s    ",  
           "score",  "bias",   "   Evalue", "hmm-from", " hmm-to", " ali-from", "   ali-to", " env-from", "   env-to")  < 0)
            ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
	  ...
	  for (d = 0; d < th->hit[h]->ndom; d++)
	  {
	    if (th->hit[h]->dcl[d].is_reported)
	    {
		  ...
		  if (pli->spliced)
          {
            if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %9" PRId64 " %9" PRId64 " %c%c %8d  %6d  %5d %9" PRId64 "   %4.2f\n",
              ...
              th->hit[h]->dcl[d].ad->L,
              (th->hit[h]->dcl[d].oasc / (1.0 + fabs((float) (th->hit[h]->dcl[d].jenv - th->hit[h]->dcl[d].ienv) / 3.0)))) < 0)
              ESL_EXCEPTION_SYS(eslEWRITE, "domain hit list: write failed");
          }
          if (pli->frameshift)
                  {
                    if (fprintf(ofp, " %c %6.1f %5.1f %9.2g %10d %9d %c%c %9" PRId64 " %9" PRId64 " %c%c %9" PRId64 " %9" PRId64 " %c%c ",
                                th->hit[h]->dcl[d].is_included ? '!' : '?',
                                th->hit[h]->dcl[d].bitscore,
                                th->hit[h]->dcl[d].dombias * eslCONST_LOG2R, /* convert NATS to BITS at last moment */
                                exp(th->hit[h]->dcl[d].lnP), // * pli->Z,
...
p7_tophits_TabularTargets(FILE *ofp, char *qname, char *qacc, P7_TOPHITS *th, P7_PIPELINE *pli, int show_header)
{
  ...
  int qaccw  = ((qacc != NULL) ? ESL_MAX(10, strlen(qacc)) : 10);
  int taccw  = ESL_MAX(10, p7_tophits_GetMaxAccessionLength(th));
  int posw   = (pli->frameshift ? ESL_MAX(9, p7_tophits_GetMaxPositionLength(th)) : 0);
  int     i,h,d;
  int     id;
  int64_t sqfrom;
  int64_t sqto;
